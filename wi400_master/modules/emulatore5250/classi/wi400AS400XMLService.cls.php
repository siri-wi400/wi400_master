...
class wi400AS400XMLService {
	...
	private $debugproc=False;

	public function __consturct () {
		global $settings;
		// Impostazione iniziali e default
	}
	...
	public function setDebugProg($debugproc) {
		$this->debugprog = $debugproc;
	}
	public function getDebugProc() {
		return $this->debufproc;
	}
	public function setPoolName($poolname) {
		$this->poolnmae = $poolname;
	}
...
function callXMLService($XInputXML, $XInternalKey=null, $XControlKey=null, $stmt=null) {
	global $db, $INTERNALKEY, $CONTROLKEY, $settings, $jobname;
	static $loop_call;
	static $libraryadd=False;
	if (!isset($stmt)) {
		$stmt = $db->getCallPGM();
		//$sql =  "call XMLSERVICE/iPLUG65K(?,?,?,?)";
		//$stmt = db2_prepare($db->getLink(), $sql);
	}
	if (!isset($XInternalKey)) {
