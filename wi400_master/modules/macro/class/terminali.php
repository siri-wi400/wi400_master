...
class terminali extends Macro {
	...
	public function generate(){
		...
		$userFileKbd = $moduli_path.'/macro/template/'.strtoupper($_SESSION['user']).'/AS400.KMP';
		if (file_exists($userFileKbd)) {
			$fileKbd = $userFileKbd;
		} else { 
		...
		$macro = 'autECLSession.autECLPS.SendKeys "'.$detailKey[0].'"
				   ...
				   autECLSession.autECLOIA.WaitForInputReady
				   autECLSession.autECLPS.SendKeys "W"
				   autECLSession.autECLOIA.WaitForInputReady
		...
		$field = array("IDOPEN","AZIONE","SYSINF","IDFL01","IDFL02","IDFL03","IDFL04","IDFL05","STATO","OKPJBU");
		$stmtInsert = $db->prepare("INSERT", "ZID5250O", null, $field);
		$campi = array($this->getId(), "£PD1", $_SESSION['sysinfname'], "S", "", "", "", "", 'V',"");
