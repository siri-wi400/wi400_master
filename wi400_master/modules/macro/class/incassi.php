...
class incassi extends Macro {
	...
	public function generate(){
		...
		$fileKbd = p13n('/modules/macro/template/AS400.KMP');		
		//$fileKbd = $moduli_path.'/macro/template/AS400.KMP'; 
		// File ICO (Default)
		$fileIco = p13n('/modules/macro/template/favicon.ico');		
		//$fileIco = $moduli_path.'/macro/template/favicon.ico'; 
		// Creazione WS
		$file = p13n('/modules/macro/template/template.ws');		
        ...
        $macro = "";
  		// Macro
		/*$macro = 'autECLSession.autECLPS.SendKeys "'.$param['ARTICOLO'].'"
		...
		$desAzienda ="";
		$stringa = str_pad($this->getParameter("AZIENDA"), 3,"", STR_PAD_LEFT).str_pad($desAzienda, 35," ", STR_PAD_LEFT).str_pad($detailKey[1], 3, STR_PAD_LEFT).
		     str_pad($detailKey[0], 8, "0",STR_PAD_LEFT)."2";
		$campi = array($this->getId(), "", "§IGEINCL", "S" ,$_SESSION['sysinfname'], "", "", "", "", "", 'V',$stringa);
