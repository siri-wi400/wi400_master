...
//						$sheet->setCellValueByColumnAndRow($col+1,$rowCounter++,$value);
		...
		$range_Header_ini = PHPExcel_Cell::stringFromColumnIndex(0).$rowHeader;
...
//										echo "WRITE_3<br>";
										$sheet->setCellValueExplicitByColumnAndRow($j,$rowHeader+$i+1,$rowValue);
...
//								$rowValue = prepare_string($rowValue, isHtml($rowValue));
								...
								if($rowFormat != ""){
									...
									$rowValue = prepare_string($rowValue);
//									$rowValue = prepare_string($rowValue, isHtml($rowValue));
									$sheet->setCellValueExplicitByColumnAndRow($j,$rowHeader+$i,$rowValue);
								}
							}
							$j++;
						}
...
//		echo "COL: $colCount<br>";
		...
		if($rowFooter>$rowData) {
			$range_Footer_ini = PHPExcel_Cell::stringFromColumnIndex(0).($rowData+1);
