--TEST--
DB creazione TABELLA PHPTEMP/TESTINST
--FILE--
<?php
...
// Verifica QUERY  	
	...
	$result = $db->query($sql);
	write_test_log("Query:".$sql);
	// Creazione della tabella
	// DEVO USARE COMANDO DI TABELLA 
	$sql = "CREATE TABLE PHPTEMP".$settings['db_separator']."TESTINST (UNO CHAR(3), DUE DECIMAL(5, 2))";
	$result = $db->query($sql);
	if (!$result) {
		die("NO");
	}
