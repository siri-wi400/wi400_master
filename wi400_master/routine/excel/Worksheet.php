...
class Spreadsheet_Excel_Writer_Worksheet extends Spreadsheet_Excel_Writer_BIFFwriter
{
    ...
    */
    ...
    /**
     * Comparazione della lunghezza della lunghezza di due stringhe
     * (utilizzata principalmente per trovare la stringa colonna di lunghezza massima in una tabella)
     *
     * @param integer $max_len	: lunghezza massima rilevata della colonna
     * @param string $string	: stringa da conrollare
     * @return integer		Ritorna la lunghezza della colonna
     */
	public function len_col_compare($max_len, $string) {
		$col_len = strlen($string);
	...
	/**
	 * Autofit delle colonne di una tabella XLS
	 *
	 * @param array $array_max_len	: array delle lunghezze massime delle colonne della tabella
	 */
	public function autofit($array_max_len) {
		for($j=0; $j<count($array_max_len); $j++)
			$this->setColumn($j,$j,$array_max_len[$j]);
	}
