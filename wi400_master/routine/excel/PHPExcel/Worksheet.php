...
class PHPExcel_Worksheet implements PHPExcel_IComparable
{
   ...
   /**
     ...
     * @return PHPExcel_Worksheet;
     */
    // SIRI: Modifica per poter settare dei margini diversi per la cella
    public function calculateColumnWidths($calculateMergeCells = false)
    {
        ...
        foreach ($this->getColumnDimensions() as $colDimension) {
            if ($colDimension->getAutoSize()) {
                ...
                $margins[$colDimension->getColumnIndex()] = $colDimension->getMargin();		// SIRI
            }
        }
     ...
     * @return PHPExcel_Worksheet|PHPExcel_Cell    Depending on the last parameter being specified
     */
    // SIRI: Modifica per poter sistemare le stringhe con caratteri particolari
    ...
    {
// SIRI INIZIO
    	if($pDataType == PHPExcel_Cell_DataType::TYPE_STRING) {
    		$pValue = prepare_string($pValue);
    	}
// SIRI FINE
...
// SIRI INIZIO
    ...
    public function deleteComment($pCellCoordinate = 'A1') {
    	unset($this->_comments[$pCellCoordinate]);
    }
    
    public function deleteCommentByColumnAndRow($pColumn = 0, $pRow = 0) {
    	unset($this->_comments[PHPExcel_Cell::stringFromColumnIndex($pColumn) . $pRow]);
    }
// SIRI FINE
