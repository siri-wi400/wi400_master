...
// SIRI - INIZIO
	/**
	 * Aggiustamento della larghezza della cella (necessario per evidenziare il testo correttamente)
	 * @protected numeric
	 */
	protected $adjCellW = 0;
// SIRI - FINE
...
// SIRI - INIZIO
	// SIRI: Aggiunta stampa degli * all'inizio e alla fine nel codice sotto le barre del barcode
	protected $print_asterisk = false;
// SIRI - FINE
	 ...
	 */
// SIRI: Cambiata l'impostazione per velocizzare il funzionamento
...
// SIRI - INIZIO
	/**
	 * Funzione per settare il valore di aggiustamento della larghezza della cella (necessario per evidenziare il testo correttamente)
	 * @param numeric $adj
	 */
	public function setAdjustCellWidth($adj=0) {
		$this->adjCellW = $adj;
	}
	
	/**
	 * Funzione per recuperare il valore di aggiustamento della larghezza della cella (necessario per evidenziare il testo correttamente)
	 * @return numeric value
	 */
	public function getAdjustCellWidth() {
		return $this->adjCellW;
	}
// SIRI - FINE
...
// SIRI - INIZIO
	/**
	 * Funzione per il recupero dei valori che interessano il pagebreak
	 * @return array
	 */
	public function GetAutoPageBreakArray() {
		return array("AutoPageBreak" => $this->AutoPageBreak, "bMargin" => $this->bMargin, "PageBreakTrigger" => $this->PageBreakTrigger);
	}
// SIRI - FINE
	...
	public function Write($h, $txt, $link='', $fill=false, $align='', $ln=false, $stretch=0, $firstline=false, $firstblock=false, $maxh=0, $wadj=0, $margin='') {
		...
		while ($i < $nb) {
			...
			if ($c == 10) { // 10 = "\n" = new line
				...
				// SIRI - Aggiustamento della larghezza della cella (necessario per evidenziare il testo correttamente)
				$w += $this->getAdjustCellWidth();
...
// SIRI - INIZIO
					// SIRI - Modifica per migliorare i tempi di generazione del file PDF, il Font Type rallenta di molto le prestazioni
...
// SIRI - INIZIO
// Funzioni tolte in nuove versioni di TCPDF perchè deprecate, 
// reinserite per far funzionare il programma con la versione di FPDI che abbiamo
// (funzione _getstream() chiamata in fpdi2tcpdf_bridge.php
	/**
...
// SIRI - INIZIO
	// SIRI: Aggiunta stampa degli * all'inizio e alla fine nel codice sotto le barre del barcode
	public function set_print_asterisk($print) {
		$this->print_asterisk = $print;
	}
	
	public function get_print_asterisk() {
		return $this->print_asterisk;
	}
// SIRI - FINE
	...
	public function write1DBarcode($code, $type, $x='', $y='', $w='', $h='', $xres='', $style='', $align='') {
		...
		if ($style['text']) {
			...
			$this->SetCellPadding(0);
// SIRI - INIZIO
			// SIRI: Aggiunta stampa del checkdigiit nel codice sotto le barre del barcode
//			$this->Cell($txtwidth, '', $label, 0, 0, 'C', false, '', $style['stretchtext'], false, 'T', 'T');
			$bar_text = $code.$barcodeobj->get_checkdigit();
			// SIRI: Aggiunta stampa degli * all'inizio e alla fine nel codice sotto le barre del barcode
//			if(in_array($type, array("C39", "C39+", "C39E", "C39E+"))
			if($this->get_print_asterisk()===true)
				$bar_text = "*".$bar_text."*";
			$this->Cell($txtwidth, '', $bar_text, 0, 0, 'C', 0, '', $style['stretchtext'], false, 'T', 'T');
// SIRI - FINE
			$this->cell_padding = $cellpadding;
		}
