	...
	public static function getConfigProperties() {
		$properties = array(
			...
			array('name' => 'zero-line-alpha', 
				  ...
				  'type' => 'numeric', 
				  'default' => 0),
			array('name' => 'avarage-line-visible', 
				  'description' => 'Set average line visibility', 			
				  'type' => 'boolean', 
				  'default' => true),
			array('name' => 'avarage-line-color', 
				  'description' => 'Average line color (Use HEX)', 			
				  'type' => 'color', 
				  'default' => '0000FF'),
			array('name' => 'avarage-line-alpha', 
			...
			array('name' => 'box-background-alpha', 
				  ...
				  'type' => 'numeric', 
				  'default' => 0),
			array('name' => 'column-devider-color', 
				  'description' => 'Column devider color (Use HEX)',			
				  'type' => 'color', 
				  'default' => '000000'),
			array('name' => 'column-devider-alpha', 
				  'description' => 'Column devider alpha',			
				  'type' => 'numeric', 
				  'default' => 100),
			array('name' => 'column-devider-visible', 
				  'description' => 'Set column devider visibility',			
				  'type' => 'boolean', 
				  'default' => true),
			array('name' => 'horizontal-devider-color', 
				  'description' => 'Horizontal devider color (Use HEX)',			
				  'type' => 'color', 
				  'default' => '000000'),
			array('name' => 'horizontal-devider-alpha', 
				  'description' => 'Horizontal devider alpha',			
				  'type' => 'numeric', 
				  'default' => 100),
			array('name' => 'horizontal-devider-visible', 
				  'description' => 'Set horizontal devider visiblity',			
				  'type' => 'boolean', 
				  'default' => true),
...
//-------------------------------------------------------------------------+
	 ...
	 * @exception: yes
	 * @since: version 5.0.1
	 * @comment: Parses a data array in to graph structure
	*/	
	...
	*/	
	protected function drawValueLabels($graphPoints, $dataStructure) {
		...
		if($this->cfg['value-label-visible']) {
			...
			//Draw strings
			imagestring($this->ip, $this->cfg['value-label-font-size'], $graphPoints['value']['min']['x1'], $graphPoints['value']['min']['y1'], $dataStructure['fakeMin'], $valueLabelColor);
			imagestring($this->ip, $this->cfg['value-label-font-size'], $graphPoints['value']['max']['x1'], $graphPoints['value']['max']['y1'], $dataStructure['fakeMax'], $valueLabelColor);
		}			
...
//-------------------------------------------------------------------------+
	...
	/**
	 * @name: drawHorizontalDeviders
	 * @param: (array) $graphPoints
	 * @return: void
	 * @access: protected
	 * @exception: no
	 * @since: version 5.0.1
	 * @comment: Draws the horizontal deviders
	*/	
	protected function drawHorizontalDeviders($graphPoints) {
		//Test for visible
		if($this->cfg['horizontal-devider-visible']) {
			//Set offset
			$offset = ($graphPoints['box']['height'] / 4);
			//Allocate colors
			$color = $this->allocateColor($this->cfg['horizontal-devider-color'], $this->cfg['horizontal-devider-alpha']);
...
//-------------------------------------------------------------------------+
	...
	*/	
	protected function drawAverage($graphPoints, $dataStructure) {
		//Test visible
		if($this->cfg['avarage-line-visible']) {
			//allocateColor
			$avgLineColor = 
				$this->allocateColor($this->cfg['avarage-line-color'], 
									 $this->cfg['avarage-line-alpha']);
	...
	*/	
	public function drawColumns($graphPoints, $dataStructure, $compareStructure = false) {
		...
		foreach($graphPoints['columns'] as $colPoint) {
			...
			//Test for visible
			if($this->cfg['column-devider-visible']) {
				//Do not print first col
				if($colIdx!=1) {
					if(!$compareStructure) {
						//Allocate colors
						$columnDeviderColor = $this->allocateColor($this->cfg['column-devider-color'], $this->cfg['column-devider-alpha']);
						$columnDeviderAlphaColor = $this->allocateColor($this->hex2Rgb('FFFFFF'), 127);
						//Define style
						$dottedStyle = array($columnDeviderColor, $columnDeviderColor, $columnDeviderAlphaColor, $columnDeviderAlphaColor);
						//Set style
	...
	*/	
	public function parse($data, $cfg = array()){
		try {
			...
			$this->drawHorizontalDeviders($graphPoints);
...
//-------------------------------------------------------------------------+
	...
	/**
	 * @name: drawVerticalDeviders
	 * @param: (array) $graphPoints
	 * @return: void
	 * @access: protected
	 * @exception: no
	 * @since: version 5.0.1
	 * @comment: Draws the vertical deviders
	*/	
	protected function drawVerticalDeviders($graphPoints) {
		//Test for visible
		if($this->cfg['horizontal-devider-visible']) {
			//Set offset
		...
		foreach($graphPoints['columns'] as $colPoint) {
			...
			if($this->cfg['key-visible']) {
				if(!$compareStructure) {
					...
					if($printCol) {
						imagestring($this->ip, $this->cfg['key-font-size'], $colPoint['key']['x1'], $colPoint['key']['y1'], $colPoint['key']['text'], $keyColor);
					}
				}
			}
	...
	*/	
	public function parse($data, $cfg = array()){
		try {
			...
			$this->drawBoxBackground($graphPoints);
			$this->drawColumns($graphPoints, $dataStructure);
			$this->drawVerticalDeviders($graphPoints);
			$this->drawLabel($graphPoints);
			$this->drawBoxBorder($graphPoints);
			$this->drawAverage($graphPoints, $dataStructure);
			$this->drawZero($graphPoints);
