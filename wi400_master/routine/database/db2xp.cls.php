...
class DB2XP {
	...
	function make_connection()
	{
		...
		if (!isset($this->link))
		{
			...
			$this->sSq($msg);
			//$this->link = db2_connect($this->db_host, "WI400", "WI400");
			// connect to DB2AS400
			if ($this->connType=='T') {
				$this->link = db2_connect($this->db_host, $this->db_user, $this->db_pwd);
			} else {
				$this->link = db2_pconnect($this->db_host, $this->db_user, $this->db_pwd);
			}
...
//			$this->setSchema("PHPLIB");
			...
			//$this->query("set transaction isolation level NONE");
	...
	function query($sql, $scrollable=True, $optimize=10)
	{
		...
		$operazione = strtoupper(substr($sql, 0, 6));
		if (in_array($operazione, array("INSERT", "UPDATE", "DELETE"))) {
		   $sql .= " WITH NC";
		}
		//$sql .= " WITH NC"; 
