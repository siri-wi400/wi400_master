...
class GENERIC_PDO_ORACLE implements WI400_DB_PDO{
	private $db_host;
	private $db_user;	
	private $db_pwd;
	private $db_name;
...
//		 	$sql = "SELECT ".substr($sql, 6);
//		 	$sql = "SELECT ".substr($sql, 6)." AND ROWNUM <= 1"; 
			// Seleziona una riga
...
//		$stmt = $this->link->prepare($sql,array(PDO::ATTR_CURSOR => PDO::CURSOR_SCROLL));
	...
	function execute(&$stmt, $campi = array() , $utfDecode=False)
	{
		...
		// Se inserimento/modifica devo fare delle considerazioni
		$insert="";
		$queryp = $stmt->queryString;//echo $queryp;
		if (strtoupper(substr($queryp, 0, 6))=="INSERT" || strtoupper(substr($queryp, 0, 12))!="UPDATE"){
			$insert="i";
		}
		...
		foreach ($campi as $key => $value) {
			...
			// In inserimento/Modifica aggiungo delle conversioni
			if ($insert == "i"){
			// Se valore numerico a zero
			if ($value == "0") $value = "0";
			// Nell'insert ORACLE non vuole il separtore delle migliaia
			$value = str_replace(",","",$value);
			// Se passato valore nullo stringa imposto uno spazio a blank
			if ($value == "") $value = " ";
//			// Se valore data passato errato
//			if ((substr($value,11,8)) == "00:00:00") $value = substr($value,0,10);
			}
			$stmt->bindValue($rr, $value, $var_type);
			$rr++;
		}
...
//				$valore = getDb2Timestamp();
	...
	function convertDataTypeString($string, $what='TO_NUMERIC') {
		...
		static $array = array("CHAR"=>"1","NUMBER"=>"3","DATE"=>"8","VARCHAR2"=>"12", "VARCHAR"=>"12","TIMESTAMP"=>"88", "TIMESTAMP(6)"=>"88","NCHAR"=>"1","NVARCHAR2"=>"1", "CLOB"=>"12");
...
//		$sql = "select TABLE_SCHEMA, TABLE_NAME from {$this->schema_info}".$settings['db_separator']."{$this->schema_tables} where TABLE_SCHEMA ='".$settings['db_temp']."' and TABLE_NAME like'%".strtoupper($session_id)."%'";
		$sql = "select OWNER from ALL_CATALOG where TABLE_NAME=ABLE_NAME like'%".strtoupper($session_id)."%' and OWNER ='".$settings['db_temp']."'";
...
//		$sql = strtoupper($sql);
		// Se trovo un doppio identificatore messo messo dalla paginazione
