...
class DB2AS400 implements WI400_DB {
	...
	function make_connection()
	{
		...
		if (!isset($this->link))
		{
			...
			//unset($tempoptions['i5_libl']);
			//if (isset($settings['db_encode_input_library']) && $settings['db_encode_input_library']==True) {
				$tempoptions['i5_libl']=utf8_decode($this->options['i5_libl']);
			//}
			//echo mb_detect_encoding($tempoptions['i5_libl']);die("fine detect");
			//$tempoptions['i5_libl']=utf8_decode($tempoptions['i5_libl']);
			//die($tempoptions['i5_libl']);
			$tempoptions['i5_commit']=DB2_I5_TXN_NO_COMMIT;
			$tempoptions['i5_naming']=DB2_I5_NAMING_ON;
	 ...
	 */
	function query($sql, $scrollable=False, $optimize=10, $startFrom=null)
	{
		global $settings;
...
//		if (strpos(strtoupper($sql), "QTEMP")!==false) {
		...
		if ($scrollable && $this->getDBAttribute("DB_SUPPORT_PAGINATION")==False) {
...
//			if (isset($messageContext)) {
			if ($this->debug && isset($messageContext)) {
				$messageContext->addMessage("ERROR",db2_stmt_errormsg());
...
//			if($this->write_logs===true)
	...
	{	
		...
		if (get_resource_type($result)=="DB2 Statement") {
			...
			if (db_cursor_type($result)==DB2_FORWARD_ONLY) {
...
//			echo "<b>QUERY_FIND:</b>$querycount<br>";
	...
	function rtvLibre ($tabella) {
		...
		if (isset ( $_SESSION ['array_librerie'][$tabella])) {
			...
			$do = $this->execute($statement, array($tabella, $tabella));
			if ($do) {
	...
	function add_to_librarylist($libraries, $forceCall=True)
	{
		...
		//$stringa = '"'.str_replace(" ", '" "', trim($stringa)).'"';
		$this->options['i5_libl']=$stringa;
		//$stringa = '"'.str_replace(" ", '" "', trim($stringa)).'"';
		if ($forceCall) {
			//die("PASSO DI QUA:".$stringa);
			$this->make_connection();
			$sql = "CALL QGPL".$settings['db_separator']."ZDT_LIBR('".trim($stringa)."')";
			$sql = "CALL QCMDEXC('CHGLIBL LIBL(".trim($stringa).")')";
			//echo $sql;
			//die ($sql);
			//$this->esegui($sql);
			db2_exec($this->link, utf8_decode($sql));
	...
	public function createTable($table, $libre="", $array, $checkExist = false, $not_null=false, $max_record="")
	{
		global $settings;
	...
	public function ifExist($table, $libre=null)
	{
		...
		if (isset($libre) AND $libre!="") {
			$sql = "select TABLE_SCHEMA, SYSTEM_TABLE_NAME from QSYS2".$settings['db_separator']."SYSTABLES where TABLE_SCHEMA ='$libre' and TABLE_NAME='".strtoupper($table)."'";
		} else {
			$sql = "select TABLE_SCHEMA, SYSTEM_TABLE_NAME from QSYS2".$settings['db_separator']."SYSTABLES where TABLE_SCHEMA IN(".$this->in_libl. ") and TABLE_NAME='".strtoupper($table)."'";
		}
...
//		if ($this->debug) {
	...
	public function set_write_logs($write_logs) {
		$this->writre_logs = $write_logs;
	}
	
	public function get_write_logs() {
		return $this->write_logs;
	}
}
?>
