...
class PDO_IBM implements WI400_DB {
	private $db_host;
	private $db_user;
	private $db_pwd;
	private $db_name;
	...
	function make_connection()
	{
		...
		if (!isset($this->link))
		{
			...
			$this->link = new PDO("ibm:*LOCAL","QPGMR","ALDEBARAN",array(
    PDO::ATTR_PERSISTENT => false, PDO::I5_ATTR_DBC_SYS_NAMING => true,
	...
	do  {
		...
		if (!$stmt) {
			...
			if ($errorCode=="42704") {
			  ...
			  //}
			  if ($find !==False) {
				   $file = trim(substr($error[2], 0, $find));
			  	   /*if ($metodo==1) {
				  	   	$find = strpos($error[2],'-');
...
function getSequence($fileNumeratore, $numeratore) {
	function getSequence($fileNumeratore, $numeratore) {
		...
		} elseif ($fileNumeratore=="ZSYSNUM") {
			return getSysSequence_OS400($numeratore);
		}
	}
}
static function create_descriptor_OS400($file, $connzend, $libre = Null, $desc = False) {
	global $db, $settings;
