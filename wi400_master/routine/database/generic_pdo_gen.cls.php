...
class GENERIC_PDO implements WI400_DB_PDO{
	private $db_host;
	private $db_user;
	private $db_pwd;
	private $db_name;
	...
	public  $schema_info = "QSYS2";
	public  $schema_tables = "SYSTABLES";
	public  $schema_columns = "SYSCOLUMNS";
...
//			echo "<b>QUERY_FIND:</b>$querycount<br>";
	...
	public function ifExist($table, $libre=null)
	{
		...
		if (isset($libre) AND $libre!="") {
			$sql = "select TABLE_SCHEMA from {$this->schema_info}".$settings['db_separator']."{$this->schema_tables} where TABLE_SCHEMA ='$libre' and TABLE_NAME='".strtoupper($table)."'";
		} else {
			$sql = "select TABLE_SCHEMA from {$this->schema_info}".$settings['db_separator']."{$this->schema_tables} where TABLE_SCHEMA IN(".$this->in_libl. ") and TABLE_NAME='".strtoupper($table)."'";
		}
	...
	public function getTableDescription($table)
	{
		...
		$this->make_connection();
		$sql = "select TABLE_TEXT FROM {$this->schema_info}".$settings['db_separator']."{$this->schema_tables} where TABLE_SCHEMA IN(".$this->in_libl. ") and TABLE_NAME='".strtoupper($table)."'";
		//$result = $this->singleQuery($sql, False);
...
function getCurrentOpenFile() {
	...
	if ($result) {
		// Ciclo sulle righe reperite
//		$query = "   SELECT TABLE_NAME, TABLE_SCHEMA FROM #lastQueryStats, PHPLIB.INFORMATION_SCHEMA.TABLES tab WHERE CHARINDEX( tab.TABLE_NAME, sql_statement) > 0";
		$query = "   SELECT TABLE_NAME, TABLE_SCHEMA FROM #lastQueryStats, ".$settings['db_name'].".INFORMATION_SCHEMA.TABLES tab WHERE CHARINDEX( tab.TABLE_NAME, sql_statement) > 0";		
		$result = $this->query($query);
		if ($result) {
			while ($row = $this->fetch_array($result)) {
				$returnFile[$row['TABLE_NAME']]=$row['TABLE_SCHEMA'];
			}
		}
	}
	// Cancellazione tabella creata al volo
	$query = "DROP TABLE #lastQueryStats ";
	//$result = $this->query($query);
	return $returnFile;
}
