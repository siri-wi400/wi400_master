...
class DB2AS400 implements WI400_DB {
	...
	function exec($query, $option=null) {
		if (isset($option)) {
			return db2_exec($this->link, utf8_decode($query), $option);
		} else {
			return db2_exec($this->link, utf8_decode($query));
		}
	}
	/**
...
//		if (strpos(strtoupper($sql), "QTEMP")!==false) {
	...
	function prepare($operazione, $file, $key=null, $field=null, $numRows=1)
	{
		...
		$stmt = db2_prepare($this->link, utf8_decode($sql));
		if (!$stmt)
		{
...
//			echo "<b>QUERY_FIND:</b>$querycount<br>";
	...
	{
		$dati = array();
		$dati['NAME']= utf8_encode(db2_field_name($result, $field));
