...
class EasyGoogleMap {
	...
	/**
	 * @desc: 	Indirizzo del centro della mappa
	 * @type: 	string
	 * @access: private
	 */
	var $center; 
	 ...
	 * @return: void
	 */
	function SetAddress($address, $isCenter=False) {
		$this->mIndex++;
	...
	function InitJs() {
		...
		if ($is_error) { 
	        ...
	        # cemtro la mappa
	        //$ret .= "map.setCenter(new GPoint( 44.00 ,13.00 ), ".$this->mMapZoom.");\n";
			...
			$ret .= "	var icon = new GIcon(); \n";
								   // http://google.webassist.com/google/markers/$dir/$color.png
			$ret .= "	icon.image = 'http://www.siri-informatica.it/google/flag_".$settings['temaDefault'].".png'; \n";
			...
			$ret .= " setTimeout(\"geoLoader(geocoder, address_$i, $i, points, gmarkers)\",".($i*250).");";
			
			} # end for
			//$ret .= " setTimeout(\"geoLoader(geocoder, address_0, 0, points, gmarkers)\",1000);";
			$ret .= "} // end if\n\n";
			...
			$ret.= "function geoLoader(geocoder, addressObj, countMark, points, gmarkers){\n";
			//$ret .= " alert(addressObj.full);\n";
				$ret .= "	geocoder.getLatLng (\n";
				$ret .= "	  addressObj.full,\n";
				$ret .= "	  function(point) {\n";
				$ret .= "		if(point) {\n";
				$ret .= "		  points[countMark] = point; \n";	
				...
				$ret .= "		  gmarkers[countMark] = marker;\n";
				$ret .= "		}\n";
				$ret .= "		else {\n";
				$ret .= "		  alert (addressObj.full + ' non trovato');\n";
				$ret .= "		}\n";
				$ret .= "	  }\n";
				$ret .= "	); // end geocoder.getLatLng\n\n";
     $ret .= "} /*end function geoLoader*/\n";
			$ret .= "</script>\n";
		} # end if
	...
	function GetSideClick() {
		...
		$ret .=	"<table class=\"wi400-grid-menu\">";
		$ret .=	"<tr>";
		$ret .=	"<td class=\"text-label\">Seleziona destinatario:</td>";
		$ret .=	"<td><select class=\"select-field\" size=\"1\" ";  
		$ret .=	"	onChange=\"sideClick(this.value)\">";
		for ($i=0; $i<$loop; $i++) {	
		$ret .=	"	<option value=\"".$i."\">".$this->mSideClickArr[$i]."</option>";
		}
		$ret .=	"				</select></td>";
		$ret .=	"				</tr>";
		$ret .=	"			</table>";
	 ...
	 */
	function MapHolder() {
		return "<div id=\"map\" class=\"work-area\" style=\"width: ".$this->mMapWidth."px; height: ".$this->mMapHeight."px;\"></div>";
	} # end function
	...
	/**
	 * @desc: 	Setto il centro della mappa
	 * @access: public
	 * @return: string
	 */
	function setCenter($indirizzo) {
          $this->center=$indirizzo;
	} # end function
	/**
