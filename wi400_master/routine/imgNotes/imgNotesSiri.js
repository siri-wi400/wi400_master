 ...
 */
			onAdd: function(dati) {
				if(typeof(dati.html) == "undefined") { 
					dati = {"classe": "black", "html": this.noteCount};
				}
				this.options.vAll = "bottom";
				this.options.hAll = "middle";
				return  $(document.createElement('span')).addClass("marker "+dati.classe).html(dati.html);
			},
/*
			...
			onEdit: function(ev, elem) {
				...
				return $('<div id="NoteDialog"></div>').dialog({
					...
					open: function(event, ui) {
						$(this).css({"overflow": "hidden", "font-size": "11px"});
						//$(this).css("font-size", "11px");
						
						var that = this;
						var note = $elem.data("note");
						if(note) {
							var dati = $elem.data("dati");
							note += "<br/><a class=\"eliminaCoordinate\" onClick=\"eliminaCoordinate('"+dati['SOCIETA']+"', '"+dati['SITO']+"', '"+dati['DEPOSITO']+"', '"+dati['PORTA']+"')\">Elimina segnalino porta</a>";
							$(this).html(note);
						}else {
							var coord = (+$elem.data('relx')).toPrecision(7);
							coord += ","+(+$elem.data('rely')).toPrecision(7);
							var url = _APP_BASE + APP_SCRIPT + "?t="+CURRENT_ACTION+"&DECORATION=iframe&f=DETAIL_MARKER&COORDINATE="+coord+"&"+jQuery("#" + APP_FORM).serialize();
							
							var html_iframe = $('<iframe scrolling="no" style="border: none;" id="prova" width="100%" height="100%" src="'+url+'">');
 ...
 *	Add a note
 */
		addNote: function(relx, rely, text, html, classe, dati) {
			var self = this;
			this.noteCount++;
			var datiElem = {"relx": relx, "rely": rely, "note": text, "html": html, "classe": classe, "dati": dati};
			var elem = this.options.onAdd.call(this, datiElem);
			var $elem = $(elem);
 ...
 */
		import: function(notes) {
			var self = this;
			$.each(notes, function() {
				self.addNote(this.x, this.y, this.note, this.html, this.classe, this.dati);
			});
			$(this.img).imgViewer("update");
		},
/*
