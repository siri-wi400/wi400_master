(function ($) {
    $.widget("extend.sorttable", $.ui.sortable, {
        ...
        //FUNZIONE SIRI
        _prevAll: function(prevAll) {
        	//var objPrev = [];
        	var num_col = 0;
        	 prevAll.each(function(j, obj) {
             	if(obj.tagName == "TD") {
             		//jQuery('th[gruppo="'+(obj.getAttribute('gruppo'))+'"]')
             		num_col = num_col + jQuery('th[gruppo="'+(obj.getAttribute('gruppo'))+'"]').length;
             	}else {
             		num_col++;
             	}
             });
        ...
        _createHelperTable: function (e, ui) {
            ...
            var index = this._prevAll(hc.prevAll()) + 1;
        ...
        _createPlaceholder: function (that) {
            ...
            if (o.helper == 'table') {
                ...
                //SIRI per il rowspan
                var row_span = self._sortCells[0].attr("rowspan");
                p.attr('rowSpan', row_span);
            }
        },
        ...
        _bubbleCols: function () {
            ...
            */
            var trs = this._table.children().find('> tr:not(.ui-sortable,.wi400-grid-header)'); //SIRI
            if (from < to) {
        ...
        _sortStart: function () {
            ...
            return function (e, ui) {
                _this._startIndex = _this._prevAll(ui.item.prevAll()) + 1;
            }
        },
        _sortUpdate: function () {
            var _this = this;
            return function (e, ui) {
                _this._endIndex = _this._prevAll(ui.item.prevAll()) + 1;
