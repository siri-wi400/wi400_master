...
objStyle: new wi400Map(),
objTabs: new wi400Map(),
serverErrors: new wi400Map(),
id: "",
//oldvalue:  "",
startFrom: 0,
...
// IMPLEMENTAZIONE WI400
// Aggiunto tabId che se settato filtra il controlli solo nella scheda corrente
//performCheck: function (formName, strRules, alertType, filterErrorsByName) {

performCheck: function (formName, strRules, alertType, filterErrorsByName, tabId) {

	cleanErrorTab(yav.id, tabId);
// FINE IMPLEMENTAZIONE WI400
    ...
    if (rules.length) {
		for(var i= 0; i<rules.length; i++) {
            ...
            if ( anErr!=null ) {
				...
				//IMPLEMENTAZIONE WI400
            	hasErrorTab(yav.id, yav.objTabs.get(aRule.el), tabId);
            	// FINE IMPLEMENTAZIONE WI400
...
init: function (formName, strRules) {
	...
	if (strRules.length) {
		...
		// Aggiunto puntatore startFrom per init lanciati più volte
        for(var i=yav.startFrom; i<strRules.length; i++) {
        	// FINE IMPLEMETAZIONE WI400
			var aRule = yav.splitRule(strRules[i]);
			if (typeof(aRule)=="undefined") continue;
			if (aRule==null) continue;
        ...
        // Implementazione wi400
        	// Incremento puntatore startfrom
        yav.startFrom = strRules.length;
        // FINE IMPLEMENTAZIONE wi400
...
performEvt: function(name) {
	...
	// IMPLEMENTAZIONE WI400
	// Aggiunto tabId per filtrare i controlli sul tab attivo
	var tabId = yav.objTabs.get(name);
	return yav.performCheck(yav.formEvt, yav.rulesEvt, 'inline', name, tabId); 
	// FINE IMPLEMENTAZIONE WI400
},
...
showHelpEvt: function(name) {
	// @todo Patch per HELP multifield da rivedere ...
    for(var i=0; i<yav.helpEvt.length; i++) {
    ...
    if ( messages!=null && messages.length>0 ) {
        ...
        for (var i=0; i<messages.length; i++) {
            ...
            if ( yav.get(yav_config.errorsdiv+'_'+elName) ) {
                ...
                // WI400 IMPLEMENTAZIONE
                //yav.objStyle.put(elName, yav.get(elName).className);
                yav.get(elName).className = yav.get(elName).className + " errorField";
            	// WI400 FINE IMPLEMENTAZIONE
            } else {
        ...
        if ( idName.indexOf(yav_config.errorsdiv+'_')==0 ) {
			if (yav.filterByName!=null) {
				if ( idName==yav_config.errorsdiv+'_'+yav.filterByName ) {
				    // WI400 IMPLEMENTAZIONE
				    if (yav.objStyle.get(yav.filterByName) != "" && yav.objStyle.get(yav.filterByName) != null){
    					yav.get(yav.filterByName).className = yav.objStyle.get(yav.filterByName);
					}
					// WI400 FINE IMPLEMENTAZIONE
...
//			customFunction = customFunction.replace("this", myRule.el); 
...
checkText: function (el, myRule) {
    ...
    if (myRule.ruleName=='required') {
    	//alert(el.id);
    	var funzioni = document.getElementById(el.id).getAttribute('onchange');
    	var n = funzioni.indexOf("multiFieldAddRemove"); 
    	if (n > 0) {
    		//alert("E' un campo mulifield");
    		var lis = document.getElementById(el.id+"_PARENT").getElementsByTagName("li");
    		if (lis.length==0) {
	            yav.highlight(el, yav_config.inputclasserror);
	            err = myRule.alertMsg;   			
    		}
    ...
    } else if (myRule.ruleName=='IPcmp') {
        reg = new RegExp(yav_config.IPcmp_regex);
        if ( !reg.test(el.value) ) {
            yav.highlight(el, yav_config.inputclasserror);
            err = myRule.alertMsg;
        }
    ...
    } else if (myRule.ruleName=='time') {
        err = yav.checkTime(el, myRule);
...
checkDate: function (el, myRule) {
    error = null;
    if (el.value!='' && el.value != yav_config.DATE_ESCAPE) {
...
checkTime: function (el, myRule) {
    error = null;
    if (el.value!='') {
        var timeFormat = yav_config.TIME_FORMAT;
        hhReg = new RegExp("hh");
        mmReg = new RegExp("mm");
        if ( !hhReg.test(timeFormat) || !mmReg.test(timeFormat) ) {
            yav.debug('DEBUG: locale format ' + timeFormat + ' not supported');
        } else {
            hhStart = timeFormat.indexOf('hh');
            mmStart = timeFormat.indexOf('mm');
        }
        strReg = timeFormat.replace('hh','[0-9]{2}').replace('mm','[0-9]{2}');
        reg = new RegExp("^" + strReg + "$");
        if ( !reg.test(el.value) ) {
            yav.highlight(el, yav_config.inputclasserror);
            error = myRule.alertMsg;
        } else {
            hh   = el.value.substring(hhStart, hhStart+2);
            mm   = el.value.substring(mmStart, mmStart+2);
            if ( !yav.checkhhmm(hh, mm) ) {
                yav.highlight(el, yav_config.inputclasserror);
                error = myRule.alertMsg;
            }
        }
...
getDefaultMessage: function (el, nameDisplayed, ruleName, comparisonValue) {
    ...
    if (ruleName=='required') {
        ...
        msg = yav_config.TIME_MSG.replace('{1}', nameDisplayed);
    ...
    } else if (ruleName=='IPcmp') {
        msg = yav_config.IPCMP_MSG.replace('{1}', nameDisplayed);
...
unformatNumber: function (viewValue){
    while(viewValue.indexOf(yav_config.THOUSAND_SEP)>0) 
	viewValue = viewValue.replace(yav_config.THOUSAND_SEP, "");
    var retval = viewValue.replace(yav_config.DECIMAL_SEP, ".");
    return retval;
},
